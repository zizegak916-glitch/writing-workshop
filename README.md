...
长篇记忆压缩固定走流式链路：浏览器、Go 同源流和 HTTPS 兼容桥都会在收到数据时续期空闲超时，不用固定总时长截断活跃输出。官方 DeepSeek 的默认模型 ID 跟随当前 `deepseek-v4-flash`，记忆任务默认关闭思考模式，把输出额度留给可保存的最终摘要；第三方中转不会仅因 Provider 名称而收到官方专用字段。兼容中转的嵌套增量、错误和 usage 会逐层提取；若模型只返回推理，或 Responses 因 `max_output_tokens` 未产生最终正文，界面会显示真实原因而不是笼统报“无法识别文本”。
