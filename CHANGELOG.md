...
- AI 写作卡增加“智能长篇、当前文档、当前选区、全书原文”阅读范围；新增逐章分块、章节合并、阶段合并和全书合并四级记忆任务。长篇更新覆盖每个章节，按内容指纹复用未改章节，只保存压缩记录；全书原文超出模型预算时明确阻止，不静默截断。
- 下载版真实网文 TXT 增加 UTF-8、GB18030/GBK、Big5 与 UTF-16 解码选择，并在统计前剔除网址、下载站提示、求票/更新说明、HTML 广告、连续重复章名和明显错码行；档案显示编码、清洗数量和正文保留率，重污染样本自动降级。
- 32 个 Prompt Skill 与 20 个独立 AI 功能统一进入任务登记和输出上限策略；新增约 26 万字中文输入的全入口模拟，逐项覆盖普通请求与流式请求，阻止新增 AI 调用绕过回归。
- 重构 TXT/Markdown/DOCX 项目导入：本地识别并可手动改选正文、大纲、世界观和笔记；序章与标准章名由正文解析器切分，大纲句和卷级描述不再触发切章。
- 收紧导入 AI 权限：AI 只补明确人物和完全缺失的大纲，不能用摘要、章名列表或规划节点创建、替换、补写章节；已有大纲也不再重复生成。
- 新增从首次建项目到候选写入、真实网文长期指导、Prompt Skill 三种修改模式、统一记忆、v6 备份迁移与自部署增强的完整用户教程；Pages 文档页与工作台内置帮助同步为可逐步执行的流程。
- 重做本地语料分析：扩展中文数字、阿拉伯数字和英文 Chapter 识别，统计段句分位、段中对白与轮次、行动/解释密度、章末信号、场景分隔和重复开句；大文本章节识别失败时明确降级。
- 本地分析直接生成带适用范围、证据与反例的指导卡；启用卡片会按当前 Prompt Skill 动态加入写作请求，真实网文不再只用于一次性校准。
- 新增显式 AI 深析：只在用户点击时发送不超过约 18,000 字的分层抽样，并保存结构化总结、不确定项和任务指导卡。
- 提示词工作台支持追加、修改旧指导块、整段替换、人工编辑和 AI 修改，所有应用继续保留逐 Skill 精确回退快照。
- 新增统一浏览器记忆入口，写作候选、流程候选、语料总结、指导卡和提示词候选均可记入项目；自部署增加 `/api/memories` 与后台管理页，启用记忆会进入后端 AI 请求。
- 修复浏览器记忆的内置分类被误存为备注、标题无法清空、切换项目仍显示旧项目卡片以及触屏端没有编辑入口；编辑与删除增加当前项目归属校验。自部署双写现在预先生成幂等后台 ID，后续更新、失败重试与删除作用于同一后台记录；后台失败时保留本地版本并标记待同步，不再把本地成功误报成整体失败。
- 修复 DeepSeek 官方接口在长篇记忆更新中“HTTP 200 但无法识别文本”：记忆压缩默认使用真实流式请求，官方 DeepSeek 自动关闭思考模式，避免推理内容耗尽输出额度而没有最终正文；若中转包裹了 `data/result/response/body` 或返回聚合 delta，也可提取最终文本，只有推理内容时会显示 `finish_reason` 的准确原因。
- 长篇记忆单次请求超时下限提高到 300 秒；流式超时改为收到响应头或数据块后续期的空闲超时，不再从发起请求起固定计时并在持续返回数据时误杀。Pages 桥与自部署 `/api/ai/stream` 同步接收任务输出上限和请求超时。
- 修复交叉审查发现的完整性问题：过期全书/邻章摘要不再注入写作请求；全书记忆任务固定绑定启动项目并以单个 IndexedDB 事务替换，切项目或写入失败不会串项目、留下半新半旧记录；项目包导入会重映射长篇记忆里的章节 ID。
- 修复中转嵌套 SSE 增量与 200 错误包无法识别、Responses `max_output_tokens` 诊断缺失，以及 Go/HTTP 桥持续输出仍被固定总超时切断的问题；桥异常断流会显式发送错误，不能把部分摘要当作完成结果。
- 后台项目记忆必须携带项目名，缺少 `project_name` 的 AI 运行只读取全局记忆；同一记忆 ID 不能跨项目覆盖。DeepSeek 官方识别改为校验真实主机，第三方中转不再误收 `thinking`，默认官方模型更新为 `deepseek-v4-flash`。

## Earlier Unreleased — 2026-08-23 — 多语料共识与分 Skill 校准

- 重构 32 个浏览器 Prompt Skill：每项由独立任务定义、统一证据优先级和按修改/生成/分析/策划/研究分流的执行协议组成，不再只发送一段泛化角色说明。
- 分析类先验证用户质疑是否成立，修改类执行事实保护与最小修改审查，生成类先还原人物知识和场景状态；主工作台与移动端统一使用仓库自有系统提示词。
- 多份真实网文不再简单合并全部规则；Go 引擎与 Pages 浏览器引擎都以“每个来源一票”的中位数形成基线，避免超长单本垄断提示词。
- 当样本的对白密度或段落长度明显冲突时，候选会标记弱约束，不把矛盾统计伪装成统一文风。
- `续写`、`节奏`、`对白/角色`、`润色/改写/去AI` 分别获得与任务有关的 Prompt Skill 差分；应用与回退仍逐 Skill 保存快照。
- proposal 新增 `method`、`warnings` 与 `skill_addenda`，同时保留旧 `addendum` 兼容现有客户端。
- 将“仓库自有 Go 引擎”收口为“默认 Go 编排内核 + 开放适配层”：自有主干负责作者确认、工具门和数据边界，成熟外部引擎可在完成契约审查与回归测试后接入。
- 重查 LINUX DO 佬友视频公益站与开源工具：l0veyou 标记为当前不可用，小野在线站与仓库视频能力分开描述，ZTU.AI 与派奇改为动态核验口径。
- 删除 README 与关于页里重复的依赖迁移自证文案，把许可证和必要署名收回 LICENSE / NOTICE。
- 修复上一笔远端发布中被截断的 `web/static/js/workbench.js`，恢复完整项目导入导出、编辑、候选与移动端逻辑。
