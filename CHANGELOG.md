...
- Go 引擎源码编译下限由 1.25 降至 1.21：移除 1.22/1.24 才支持的整数 `range` 与 `strings.SplitSeq`，回退终端 UI 和 Unicode 依赖到 Go 1.21 兼容线；Docker、CI 与发布验证固定在 Go 1.21.13，并以 `GOTOOLCHAIN=local` 阻止自动升级掩盖问题。Linux 发布包继续使用 `CGO_ENABLED=0`，运行时不要求预装 Go。
