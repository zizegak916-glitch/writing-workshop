...
> 状态：现行产品协议，更新于 2026-07-28（UTC+8）。v0.2 新增可选分类、标签和技能包组合，不改变 v0.1 字段语义。
...
`GET /api/external-catalog` 返回经过来源、许可证和权限初筛的 Agent Skills / MCP 上游入口。`POST /api/external-catalog` 只把选中条目登记成停用的 `external:*` capability；不下载、不安装、不启动进程。服务端拒绝通过普通 manifest 把 `external:*` 入口设为启用，也拒绝执行手工篡改为启用的外部入口。目录中的“官方参考实现”仍不等于生产可用，接入前必须固定版本并单独做威胁建模。
...
- `external:*` 入口永远是审查元数据，不属于 v0.2 执行面。
