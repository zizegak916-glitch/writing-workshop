...
> 状态：现行产品事实账本。历史条目沿用当时记录的 UTC+8；新条目使用明确时区。提交、CI、Pages 和公开页面证据必须能相互对应。最后同步：2026-08-22 UTC。

## 2026-08-22：原生引擎与授权语料校准（发布前）

| 时间（UTC） | 事件 | 当前证据状态 |
|---|---|---|
| 发布前 | 当前 Go import graph 移除 `agentcore` / `litellm`，以 `internal/engine` 实现消息、工具、上下文、协议、子任务与安全编辑；配置新写入统一到 `.writing-workshop` | 本地引擎、语料、配置与规则测试通过；全仓仅完成编译验证，等待 GitHub CI |
| 发布前 | 新增授权语料校准：原文瞬时解析，只持久化 SHA-256、元数据、聚合指标和候选；Pages 浏览器内分析，自部署使用 Go API | 静态合约与 API 适配测试通过；Playwright 因当前执行环境禁止 loopback 未在本地运行，等待 CI |
| 发布前 | 项目包升级为 v6，携带语料统计/校准历史但不含原文，兼容 v1–v5 导入 | 浏览器测试用例已写入，成功证据待 CI；没有预填提交、Actions 或 Release 链接 |

本节在合并与发布后补真实 commit、CI、Pages 和 Release 链接；在此之前不把“代码已写”记成“公开发布成功”。
