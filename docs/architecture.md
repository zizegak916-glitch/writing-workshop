...
依赖单向：`entry → host → agents → tools → store → domain`。`tools/` 不引用 `agents/host/`，`host/` 不直接引用 `tools/store/`。横向独立模块：`apperr/` 可被任何层引用，`diag/` 订阅 host 事件流 + 只读 `store/`。
...
| `edit_chapter` | drafts/chXX.draft.md | draft（digest 变化触发新 checkpoint） |
| `check_consistency` | drafts/chXX.check.json | consistency_check |
| `commit_chapter` | chapters/chXX.md + Progress | commit |
| `save_review` | reviews/vNaM.json | review |
| `save_arc_summary` | summaries/arc-vNaM.md | arc_summary |
| `save_volume_summary` | summaries/volume-N.md | volume_summary |
| `save_foundation` | foundation/*.json | premise / outline / characters / world / expand_arc / append_volume / update_compass / complete_book |

`commit_chapter` 承担弧/卷/全书完成检测，返回 17 个事实字段（`arc_end` / `needs_expansion` / `book_complete` 等）。`save_review` 承担 verdict 升级（评分卡门禁、契约 missed → rewrite）。这些过去散落在 policy 层的逻辑现在固化在工具内部。
...
```
Coordinator (1 agent run, MaxTurns=1000)
    ↓ subagent()
...
子代理 turn 计数独立（agentcore 原生），不占 Coordinator 的 1000 turn 配额。子代理之间通过 Store 中的结构化工件通信，Coordinator 只传"任务描述"不搬内容。
...
| 层 | 落点 | 作用 |
|---|---|---|
| `StopAfterTools` | `agents/build.go` SubAgentConfig | 关键工具成功 = 立即 end_turn 退出 subagent run。Writer 的 `commit_chapter`、Editor 的 `save_review`/`save_arc_summary`/`save_volume_summary` 命中即停 |
...
type Host struct {
    ...
    events, streamCh, clearCh, done chan ...
...
**观察通道**：`Events` / `Stream` / `StreamClear` / `Done`

**UI 聚合**：`Snapshot()` —— TUI 一次拉取所有展示数据

**配置/扩展**：模型管理（`SwitchModel`）、外部小说反推导入（`ImportFrom`）、共创对话（`CoCreateStream`）、事件回放（`ReplayQueue`）
...
| `Continue(text)` | 无 | 续写、停机后唤醒 | 运行中走 `FollowUp`；停机走 `Inject` 自动恢复 run |
...
internal/
  ...
  host/           host.go + resume.go + observer.go + events.go + usage.go + stream_extract.go
                  + cocreate.go
...
assets/
  prompts/        coordinator (~45 行) / architect-short|long / writer / editor / import-*
...
实测：hy3-preview free 12 章 / 73 分钟、mimo-v2.5-pro 10 章 / 8.4 万字（章均 8400），均一次跑完。长篇 100+ 章场景待补样本。
