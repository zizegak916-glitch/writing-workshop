...
- `localStorage` 不是秘密保险箱。Prompt Skill 可保存在其中，但 API Key、令牌和私密作品不应被写进提示词；导出的 v6 项目包可能含作品、笔记、记忆、AI 候选/恢复快照、分类、自定义提示词和语料聚合档案，分享前应人工检查。
- 语料分析必须要求用户确认处理权限。自部署请求总量最多 64 MiB、单文件最多 20 MiB、一次最多 20 个文件；只接受 TXT、Markdown 和 DOCX。
- 语料原文只能在本次解析内存中存在。持久化档案和项目包不得包含原文，只能包含文件哈希、元数据、聚合指标、候选规则和撤销快照。
- 校准候选不得包含具体作者仿写指令、来源专名、情节或句子；服务端 proposal 必须保持未应用状态，只有浏览器中用户确认后才能修改 Prompt Skill。
