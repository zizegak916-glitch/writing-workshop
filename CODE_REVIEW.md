...
GitHub Pages 正式在线版与可选本地服务有明确运行时边界。Pages 是真实公开域名上的正式部署，可以使用浏览器本地项目、笔记、分类与 BYOK 自定义 API；当前默认 Pages 工作流未同时部署 Go API，因此单独打开该版本时不执行 `/api/run` 或保存后端技能包。浏览器工作台和 Go 服务是两套明确存储，不再把每次浏览器编辑静默写入单个后端项目；导入后端内容必须由作者显式触发。`admin.html` 只作为自部署服务控制台存在，不属于 Pages 主导航；LINUX DO 等论坛只属于外部社区与历史发布来源。
