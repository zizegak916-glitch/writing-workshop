...
- Playwright 已覆盖 v1-v3→v4 已知字段迁移，以及“本地执行→候选→跨文档阻止→确认写入→刷新恢复候选→恢复写入前”的主链路；仍缺 Prompt Skill 覆盖值的完整浏览器迁移和真实供应商网络测试。
- OpenAI 兼容与 Anthropic 目前使用本地模拟服务做请求/响应契约测试，不使用真实密钥。它能防止适配器形状回归，但不能证明任意代理商、模型名和生产网络都可用。
