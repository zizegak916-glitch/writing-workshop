...
- 授权网文语料既可提供句段、对白、节奏等本地统计，也可提供经用户选择的场景推进、人物行动、信息释放、章节收束等可迁移指导。指导卡必须带适用范围、证据与反例；它不是情节来源、作者身份或硬配额。不得复制来源作品的专名、句子、人物、桥段，不得为追统计数字牺牲当前场景。`;
...
4. 只保留能改变当前情节、称谓、动作、物件、语言或可信度的资料，并说明具体用在哪里；不要堆百科信息。`;
  ...
  function promptText(name) {
    const base = currentSkill(name).prompt;
    const guidance = typeof window.wwCorpusGuidanceForSkill === 'function'
      ? window.wwCorpusGuidanceForSkill(name)
      : '';
