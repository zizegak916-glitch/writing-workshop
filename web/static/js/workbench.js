Warning: truncated output (original token count: 61271)
Total output lines: 5123
...
async function exportProject(){
  ...
  const d={version:6,exported_at:new Date().toISOString(),project:S.proj.project,outlines:S.proj.outlines,charact…11271 tokens truncated…   if(!text)throw new Error('后端流式连接已结束，但没有返回文本');
    return{text,usage};
  }catch(error){
...
async function doMultiGenerate(){
  ...
  if(extra)prompt+='\n\n【额外指令】'+extra;
  let sysPrompt=typeof wwSystemPrompt==='function'?wwSystemPrompt():'你是一位专业的中文写作助手。';
  const memCtx=buildMemoryContext();
  if(memCtx)sysPrompt+='\n\n'+memCtx;
  const promises=[];
  const results=[];
