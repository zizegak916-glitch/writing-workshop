...
// 从 50% 跌到 2.8%）。
//
// 注意 StopReasonError / StopReasonAborted 不需要列入：engine 在
// loop.go 收到这两种 stop reason 时直接终止 run，根本不会调用 StopGuard。
// 这里只列那些会真正走到 StopGuard 的 provider 拒答语义。
var hardStopReasons = map[engine.StopReason]struct{}{
	"safety":         {},
	"content_filter": {},
}
...
func newCheckpointDeltaGuard(st *store.Store, agentName string, requiredSteps []string, blockMsg string) engine.StopGuard {
	var baseline int64
	...
	return func(_ context.Context, info engine.StopInfo) engine.StopDecision {
		...
		slog.Warn("subagent stop_guard 拦截 end_turn",
			"module", "host.reminder", "agent", agentName, "turn", info.TurnIndex, "consecutive", n)
		return engine.StopDecision{Allow: false, InjectMessage: blockMsg}
	}
}

// NewWriterStopGuard 要求 writer 本轮至少产生一次成功的 commit_chapter。
func NewWriterStopGuard(st *store.Store) engine.StopGuard {
...
func NewArchitectStopGuard(st *store.Store) engine.StopGuard {
...
func NewEditorStopGuard(st *store.Store, task string) engine.StopGuard {
	switch {
