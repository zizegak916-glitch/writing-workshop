...
func (s *Server) resolveRunCapabilities(req runRequest) ([]capabilityManifest, error) {
	...
	for _, id := range ids {
		...
		if strings.HasPrefix(strings.ToLower(strings.TrimSpace(cap.Entry)), "external:") {
			return nil, fmt.Errorf("capability %q is metadata-only and cannot run", id)
		}
...
func validateCapability(cap capabilityManifest) error {
	...
	if strings.HasPrefix(strings.ToLower(strings.TrimSpace(cap.Entry)), "external:") && cap.Enabled {
		return fmt.Errorf("external catalog entries are metadata-only and cannot be enabled")
	}
